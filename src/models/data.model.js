...
const schema = {
    ...
    actions: Joi.array().required().items(Joi.object({
        entitledAccount: Joi.object().required().keys({
            bankID: Joi.string().required().min(2).max(2),
            branchID: Joi.string().required().min(3).max(3),
            accountNumber: Joi.string().required().min(9).max(9)
        }),
...
export default class Data {
    constructor(data) {
        ...
        const agorot = actions.reduce((total, action) => Number(action.sum.agorot), 0);
        const shekel = actions.reduce((total, action) => Number(action.sum.shekel), 0);
